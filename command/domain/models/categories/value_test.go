...
var _ = Describe("Categoryエンティティを構成する値オブジェクト", Ordered, Label("CategoryId構造体の生成"), func() {
	...
	var category_id *CategoryId // UUID文字列を指定する
	var uid string
	// 前処理
	BeforeAll(func() {
		_, empty_str = NewCategoryId("")
		_, length_over = NewCategoryId("aaaaaaaaaabbbbbbbbbbccccccccccdddddddddd")
...
var _ = Describe("Categoryエンティティを構成する値オブジェクト", Ordered, Label("CategoryName構造体の生成"), func() {
	...
	var category_name *CategoryName // 20文字以内の文字列を指定する
	// 前処理
	BeforeAll(func() {
		_, empty_str = NewCategoryName("")
		_, length_over = NewCategoryName("aaaaaaaaaabbbbbbbbbbccccccccccd")
		category_name, _ = NewCategoryName("食料品")
	})
	// 文字数の検証
	Context("文字数の検証", func() {
		It("空文字列の場合、errs.DomainErrorが返る", func() {
